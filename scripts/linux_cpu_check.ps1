...
& $PythonExecutable -m harness execute --profile ultra5060 --kind cpu --timeout 1800 -- $qemuBin -machine q35 -accel tcg -cpu max -smp 2 -m 2048 -bios (Join-Path $vmDir 'qemu/share/edk2-x86_64-code.fd') -drive "file=$overlay,if=virtio,format=qcow2" -drive "file=$vmDir/seed.iso,media=cdrom,readonly=on" -netdev user,id=net0 -device virtio-net-pci,netdev=net0 -display none -serial "file:$serial" -monitor none -no-reboot
