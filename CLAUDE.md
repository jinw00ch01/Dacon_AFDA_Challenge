...
9. **S1 방향(9/26 15:55 승인).** 공식 데이터 설명에 따르면 Baseline S1 재촬영 예제는 "모사 파생 영상"이다. 비공개 평가의 재촬영이 이 방식(디지털 잡음·재인코딩)인지, 실제 화면·기기 재촬영인지는 알 수 없다. 그러니 어느 한쪽에만 맞추지 않는다.
   ...
   - **채택 기준:** (1) Baseline 공식 예제 10개(학습 금지)는 통과 조건이다(10개 중 8개 이상). 표본이 작아 모델 간 세밀한 비교에는 쓰지 않는다. (2) 합성 검증은 v1과 v1c(코덱 동일 쌍 포함)를 따로 보고하고, 둘 중 낮은 값으로 모델을 고른다. (3) S23 실촬영이 생기면 그것을 가장 우선하는 검증 기준으로 쓴다.
