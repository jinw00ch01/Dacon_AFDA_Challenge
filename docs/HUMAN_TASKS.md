...
| `collision_frame` | 두 차량이 처음 닿는 프레임 번호. `actual_contact=yes`일 때만 쓴다 |
| `entry_frame` | 피해 차량이 상대 차량의 차선에 처음 들어오는 프레임(화면에 처음 보이는 순간이 아님). 충돌보다 앞이어야 한다 |
| `entry_side` | 화면 기준 `LEFT` / `RIGHT` |
| `evasion_space` | 충돌 순간 피할 공간이 있었으면 `1`, 없었으면 `0` |
| `lane_entry_suitable` | 차선 진입 사고로 쓸 만하면 `yes` / 아니면 `no` / 애매하면 `uncertain` |
