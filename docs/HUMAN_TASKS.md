...
DACON [Stage별 공지](https://dacon.io/competitions/official/236753/talkboard/417186)의 재녹화 정의는 "원본 영상을 별도의 화면에서 재생한 후 다른 기기로 다시 촬영한 영상"이다. 공지가 드는 특징은 화면 테두리·재생 화면 일부 노출, 화면 주사 패턴, 반사광과 주변 환경, 밝기·색상·명암 변화, 추가 압축, 촬영 기기의 움직임·원근 변화, 해상도·화면 비율 변화다. 공개 예시 클립은 공지에서 밝힌 대로 시뮬레이션이다. 그래서 S23 실촬영이 실제 평가 데이터에 가장 가까운 자료다. 아래처럼 조건별로 "깨끗한" 촬영과 "현실적인" 촬영을 나눠 이 특징들을 고르게 담는다.

자세한 설정과 품질 기준: [LABELING_AND_S23_PLAN.md](LABELING_AND_S23_PLAN.md)의 "촬영 공간과 화면", "S23 Ultra 설정", "촬영 순서와 품질 판정". 아래 2·3번의 C2·C4 방식은 그 문서보다 우선한다.
...
2. 촬영 방식은 조건에 따라 나눈다.
   - **C1·C3(정면) — 깨끗한 재촬영**: 휴대폰을 삼각대나 받침에 고정하고 렌즈를 화면 중앙 높이에 맞춘다. 25~45cm에서 시작해 영상이 화면 가로의 70~90%를 채우게 한다. 직사광과 반사를 피하고 마우스 커서·알림을 치운다.
   - **C2·C4(약 20°) — 현실적인 재촬영**: 휴대폰을 **손으로 들고** 찍는다(자연스러운 흔들림은 그대로 둔다). 화면 정면에서 옆으로 비껴 서서, **화면 테두리(베젤)나 재생 창 일부, 주변이 조금 보이게** 잡는다. 실내 조명 반사가 약간 있어도 된다. 방향(좌/우)을 기억한다.
3. 노트북 밝기를 조건대로 맞추고(자동 밝기 끔) 1배속으로 재생한다. C1·C3는 전체 화면으로 재생한다. C2·C4는 전체 화면이 아니어도 된다(재생 창 일부 노출).
